...
public class AdminCustomersServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch(action){
            case "/List":
                getViewCustomerList(request, response);
                break;
        }
    }
    
    private void getViewCustomerList(HttpServletRequest request, HttpServletResponse response) 
            throws ServletException, IOException{
        
        List<Customers> listCus =  customersFacade.findAll();
        request.setAttribute("listCus", listCus);
        dispatcher = request.getRequestDispatcher("/admin/views/customer/list.jsp");
