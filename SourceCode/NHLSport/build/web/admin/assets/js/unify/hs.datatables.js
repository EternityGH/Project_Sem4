		...
		*/
		...
		pagination: function(target, table, pagiclasses, pagiitemclasses, pagilinksclasses, paginextclasses, paginextlinkclasses, paginextlinkmarkup, pagiprevclasses, pagiprevlinkclasses, pagiprevlinkmarkup, pages) {
			...
			for (var i = 0; i < info.recordsTotal; i++) {
                            if (i % info.length == 0) {
                                    if(i / info.length == 0){
                                        paginationMarkup += '<li class="' + pagiitemclasses + '"><a id="' + target + (i / info.length) + '" class="' + pagilinksclasses + ' active" href="#!" data-dt-page-to="' + (i / info.length) + '">' + ((i / info.length) + 1) + '</a></li>';
                                    }
                                    else {
                                        paginationMarkup += i / info.length <= 4 ? '<li class="' + pagiitemclasses + '"><a id="' + target + (i / info.length) + '" class="' + pagilinksclasses + '" href="#!" data-dt-page-to="' + (i / info.length) + '">' + ((i / info.length) + 1) + '</a></li>' : '<li class="' + pagiitemclasses + ' hidden-x"><a id="' + target + (i / info.length) + '" class="' + pagilinksclasses + '" href="#!" data-dt-page-to="' + (i / info.length) + '">' + ((i / info.length) + 1) + '</a></li>';
                                    }
                            }
			}
			...
			$('#' + target + ' [data-dt-page-to]').on('click', function() {
				var $page = $(this).data('dt-page-to'),
					info = table.page.info();
                                $('#' + target + ' [data-dt-page-to]').parent().addClass('hidden-x');
                                for(var i = 1; i <= 2; i++){
                                    var intPlus = parseInt($page) + i;
                                    var intSub = parseInt($page) - i;
                                    $(this).parent().removeClass('hidden-x');
                                    $('#' + target + intPlus).parent().removeClass('hidden-x');
                                    $('#' + target + intSub).parent().removeClass('hidden-x');
                                }
			...
			$('#' + target + 'Next').on('click', function() {
				var $currentPage = $('#' + target + ' [data-dt-page-to].active');
                                ...
                                $('#' + target + ' [data-dt-page-to]').parent().addClass('hidden-x');
                                for(var i = 0; i <= 2; i++){
                                    var intPlus = parseInt($currentPage.data('dt-page-to')) + i + 1;
                                    var intSub = parseInt($currentPage.data('dt-page-to')) - i + 1;
                                    $('#' + target + intPlus).parent().removeClass('hidden-x');
                                    $('#' + target + intPlus).parent().removeClass('hidden-x');
                                    $('#' + target + intSub).parent().removeClass('hidden-x');
                                }
